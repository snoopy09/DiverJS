...
class LearningState {
	...
	get() {
		...
		// let seqFeatures;
		// seqFeatures = this._cbSeqFeatures.map(f => f(self._callbackSeqA)).map(v => typeof v === "boolean" ?  (v ? 1 : 0) : v);
		// for (let i in seqFeatures) {
		// 	features[i] += seqFeatures[i];
		// }
		// seqFeatures = this._cbSeqFeatures.map(f => f(self._callbackSeqB)).map(v => typeof v === "boolean" ?  (v ? 1 : 0) : v);
		// for (let i in seqFeatures) {
		// 	features[i] += seqFeatures[i];
		// }
