...
class LearningState {
	...
	_cbSeqFeatureFuncs () {
		return [
			...
			seq => seq.filter(e => e.name.indexOf("[req]") === 0).length <= 1, // リクエストの長さ <= 1
			seq => seq.filter(e => e.name.indexOf("[req]") === 0).length <= 3, // リクエストの長さ <= 3
			seq => seq.filter(e => e.name.indexOf("[req]") === 0).length <= 5, // リクエストの長さ <= 5
	...
	_nodeFeatureFuncs () {
		return [
			...
			node => (node.execT || node.exec) && node.distToDiff >= 0, // ノードと変更箇所に到達可能か
			node => (node.execT || node.exec) && node.distToDiff >= 0 && node.distToDiff <= 3, // ノードと変更箇所の距離が3以下
			node => (node.execT || node.exec) && node.distToDiff >= 0 && node.distToDiff <= 1, // ノードと変更箇所の距離が1以下
			node => (node.execT || node.exec) && node.diff != "unchanged", // ノードが変更されたかどうか
			...
			node => node.selected < 3 && node.distToDiff >= 0, // ノードと変更箇所に到達可能か
			node => node.selected < 3 && node.distToDiff >= 0 && node.distToDiff <= 3, // ノードと変更箇所の距離が3以下
			node => node.selected < 3 && node.distToDiff >= 0 && node.distToDiff <= 1, // ノードと変更箇所の距離が1以下
			node => node.selected < 3 && node.diff != "unchanged", // ノードが変更されたかどうか
		];
	}
	...
	get() {
		...
		for (let id in this._nodesA) {
			let node = this._nodesA[id];
			if (!("label" in node) || node.diff == "unchanged") continue;
			let nodeFeatures = this._nodeFeatures.map(f => f(node)).map(v => typeof v === "boolean" ?  (v ? 1 : 0) : v);
			for (let i in nodeFeatures) {
				features[i] += nodeFeatures[i];
			}
		...
		// ret = ret.concat(features.map(v => v === 0 ? 0 : v/Object.keys(self._nodesB).filter(id => self._nodesB[id].label).length));
		ret = ret.concat(features.map(v => v === 0 ? 0 : v/(Object.keys(self._nodesA).filter(id => self._nodesA[id].label && self._nodesA[id].diff != "unchanged").length+Object.keys(self._nodesB).filter(id => self._nodesB[id].label && self._nodesB[id].diff != "unchanged").length)));
