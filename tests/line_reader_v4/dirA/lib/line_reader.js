(function () {
    ...
    function createLineReader(fd, options, cb) {
        ...
        if (!options)
            options = {};
        var filePosition = 0, encoding = options.encoding || 'utf8', separator = options.separator || /\r\n?|\n/, bufferSize = options.bufferSize || 1024, buffer = Buffer.alloc(bufferSize), bufferStr = '', decoder = new StringDecoder(encoding), closed = false, eof = false, separatorIndex = -1, separatorLen;
        var findSeparator;
    ...
    function eachLine(filename, options, iteratee, cb) {
        ...
        var finalFn, asyncIteratee = iteratee.length == 3;
        var theReader;
        var getReaderCb;
