var util = require('util'), logger = require('./node-logger-63db153c12b2350dad8bf273b0fb537893a717c3/index.js'), assert = require('assert-plus'), fs = require('fs');
    ...
    if (this.fileBusy === true) {
        ...
        if (this.fileWriteStream.write(buffer, function (err) {
                if (self.fileSize > self.maxFileSize) {
                    self.fileSize = Number.MIN_VALUE;
...
FileEndpoint.prototype.stop = function (errCallback) {
    if (this.fileWriteStream === undefined) {
        throw new Error('Can not stop 2 times');
    }
    var file = this.file;
    var self = this;
...
function canWrite(owner, inGroup, mode) {
    return owner && mode & 200 || inGroup && mode & 20 || mode & 2;
}
