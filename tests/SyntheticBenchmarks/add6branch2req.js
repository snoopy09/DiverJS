...
var fs = require('fs');
S$.registerAsyncTasks(fs, "fs", ["readFile", "writeFile", "exists", "mkdir"]);
...
function callWriteFile(file) {
  if (file && file !== "") {
    fs.writeFile(file, "init", function (err) {
      if (err) return;
    });
  }
}
...
S$.registerRequest("writeFile", [""], callWriteFile);
S$.registerRequest("readFile", [""], callReadFile);
S$.callRequests();
