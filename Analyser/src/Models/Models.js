...
import PathModels from './JSONModels';
  ...
  JSONModels(state, ctx, model, helpers);
	...
	model.add(Object, function(base, args) {
		...
		if (!(concrete.args[0] instanceof Object) && state.isSymbolic(args[0])) {
			result = new ConcolicValue(result, state.asSymbolic(args[0]), state.asProcedure(args[0]), []);
		}
	...
	Object._expose.makeSymbolic = function(name, initial) {
		// 実行の途中でtaintでラップされちゃう
		return state.createSymbolicValue(state.getConcrete(name), state.getConcrete(initial));
	};
	Object._expose.notAnError = function() {
		 return NotAnErrorException;
	 };
	Object._expose.pureSymbol = function(name) {
		 return state.createPureSymbol(state.getConcrete(name));
	 };
	Object._expose.checkOutput = function(id, value) {
		 return state.pushOutputValue(id, value);
	 };
	Object._expose.registerRequest = function(name, args, func) {
		 return state.addRequest(name, args, func);
	 };
	Object._expose.callRequests = function(endCallback) {
		 return state.callRequests(endCallback);
	 };
	Object._expose.setAsyncHooks = function(mod) {
		return state.setAsyncHooks(mod)
	};
	return model;
}
