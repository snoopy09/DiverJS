...
export default function(state, ctx, model, helpers) {
    ...
    state.stats.seen('Regex Encoded');
		...
		// const regexEncoded_proc = new Proc("Z3.Regex", [regex]);
	...
	function NotRegexpBuiltinSplit(sep, string) {


		let results = [];

		while (true) {
			const nextIdx = model.get(String.prototype.indexOf).call(string, sep);
			if (state.getConcrete(nextIdx) >= 0) {
				const wordsBeforeSplit = model.get(String.prototype.substring).call(string, 0, nextIdx);
				results.push(wordsBeforeSplit);
				// Log.log(wordsBeforeSplit);

				string = model.get(String.prototype.substring).call(string, state.binary("+", nextIdx, 1));
			} else {
				break;
			}
		}
		...
		Log.log("NotRegexpBuiltinSplit呼んだ");
		results.forEach(s => {
			Log.log(s);
		});
	...
	function RegexpBuiltinSplit(regex, string) {
		...
		regex = state.getConcrete(regex);
		if (!(regex instanceof RegExp)) {
			return NotRegexpBuiltinSplit(regex, string);
		}
		...
		while (true) {
			...
			if (next) {
				...
				const matchSize = new ConcolicValue(
					...
					// new Proc("getLength", [state.asProcedure(next[0])]),
					// []
				);
	...
	function shouldBeSymbolic(regex, string) {
		// Log.log("shouldBeSymbolic呼んだ");
		// Log.log(regex);
		// Log.log(string);
		// Log.log(regex instanceof RegExp);
		// Log.log(state.isSymbolic(regex.lastIndex) || state.isSymbolic(string));
		if (state.getConcrete(regex) instanceof RegExp) {
			return state.isSymbolic(state.getConcrete(regex).lastIndex) || state.isSymbolic(string);
		} else {
			return state.isSymbolic(string);
		}
	}
