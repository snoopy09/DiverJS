...
class Coverage {
	...
	callCb(id) {
		this._path.push({type: "call", id: id});
	}

	touch(iid, command, op) {
		this.getBranchInfo()[iid] |= IS_TOUCHED;
		this._lastIid = iid;

		this._commandMap[iid] = op ? command+"~"+op : command;
		this._path.push({type: "touch", iid: iid, loc: this._location(iid)});
	}

	touch_cnd(iid, result) {
		this.touch(iid, "conditional");
	...
	lastLoc() {
		return this._location(this._lastIid || 0);
	}
